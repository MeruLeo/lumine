...
export default function ProjectsCompletedPage() {
  ...
  const { fetchByStatus, fetchByStatusWithModelingCode, filteredProjects } =
    useProject();
  ...
  useEffect(() => {
    if (!user) return;
    if (user.role === "model") fetchByStatusWithModelingCode("completed");
    else fetchByStatus("completed");
  }, [user]);
